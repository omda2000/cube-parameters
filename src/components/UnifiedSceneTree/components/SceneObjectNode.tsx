...
const SceneObjectNode = ({
  ...
  const isSelectedObject = isSelected(sceneObject.id);
  ...
  const handleClick = (event: React.MouseEvent) => {
    ...
    onObjectSelect(sceneObject, addToSelection);
  };
    ...
    <div>
      <div 
        className={`flex items-center py-1 px-2 hover:bg-slate-700/30 rounded text-sm cursor-pointer ${
          isSelectedObject ? 'bg-blue-600/30 border border-blue-500/50' : ''
        }`}
        style={{ paddingLeft }}
        onClick={handleClick}
      >
        <div className="flex items-center gap-1 flex-1 min-w-0">
          ...
          <span className={`truncate flex-1 ${isSelectedObject ? 'text-blue-300 font-medium' : 'text-slate-200'}`}>
            {sceneObject.name}
          </span>
